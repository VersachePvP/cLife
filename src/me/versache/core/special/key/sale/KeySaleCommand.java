	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0 || args.length > 2) {
            sender.sendMessage(
                    "§7§m--------------------------------");
            sender.sendMessage(
            		"§b/keysale start <duration> - Starts the sale timer.");
            sender.sendMessage(
            		"§b/keysale end - Ends the current sale timer.");
            sender.sendMessage(
                    "§7§m--------------------------------");
			return true;
		}
		...
		if (args.length == 1) {
			if (args[0].equalsIgnoreCase("end")) {
				if (plugin.getTimerManager().getSaleTimer().clearCooldown()) {
					...
					Bukkit.broadcastMessage(ChatColor.RED + "The keysale has been ended!");
					Bukkit.broadcastMessage(ChatColor.GRAY.toString() + ChatColor.STRIKETHROUGH
							+ BukkitUtils.STRAIGHT_LINE_DEFAULT);
					return true;
				}
		...
		if (args.length == 2) {
			if (args[0].equalsIgnoreCase("start")) {
				...
				KeySaleTimer KeySaleTimer = plugin.getTimerManager().getKeySaleTimer();

				if (!KeySaleTimer.setRemaining(duration, true)) {
				...
				Bukkit.broadcastMessage(ChatColor.GRAY.toString() + ChatColor.STRIKETHROUGH
						+ BukkitUtils.STRAIGHT_LINE_DEFAULT);
				Bukkit.broadcastMessage(ChatColor.GRAY + "The key sale " + ChatColor.GRAY + "has started! Visit " + ChatColor.GRAY + ConfigurationService.DONATE_URL
						+ ChatColor.GRAY + " (" + DurationFormatUtils.formatDurationWords(duration, true, true) + ChatColor.GRAY + ")");
				Bukkit.broadcastMessage(ChatColor.GRAY.toString() + ChatColor.STRIKETHROUGH
						+ BukkitUtils.STRAIGHT_LINE_DEFAULT);
			} else {
				sender.sendMessage(ChatColor.RED + "Unknown sub-command!");
