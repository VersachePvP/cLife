  ...
  implements CommandExecutor
{
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
  {
    if (sender instanceof Player)
    {
      ...
      {
        String msgto = MessageEvent.format("&8(&fTo " + "&f" + Bukkit.getPlayer(args[0]).getName() + "&8)&7 " + MessageEvent.toString(args, 1));
        sender.sendMessage(msgto);
        
        String msgrecieve = MessageEvent.format("&8(&fFrom &f" + sender.getName() + "&8) &7" + MessageEvent.toString(args, 1));
