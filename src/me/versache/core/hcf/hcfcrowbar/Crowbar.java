package me.versache.core.hcf.hcfcrowbar;
...
public class Crowbar {
	public static final int MAX_SPAWNER_USES = 2;
	...
	public Crowbar(int spawnerUses, int endFrameUses, int endDragonUses) {
		...
		setSpawnerUses(Math.min(2, spawnerUses));
	...
	public void setSpawnerUses(int uses) {
		if (this.spawnerUses != uses) {
			this.spawnerUses = Math.min(2, uses);
			this.needsMetaUpdate = true;
		}
	}
	...
	private static final String CROWBAR_NAME = ChatColor.GOLD + ChatColor.BOLD.toString() + "Crowbar";
